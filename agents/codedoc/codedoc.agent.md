...
When the user asks you to analyze a codebase, you execute a **multi-phase pipeline**:
...
Phase 3b: MARKDOWN → Generate a portable Markdown report (architecture-report.md)
Phase 3c: PNG      → Export Draw.io diagrams to PNG (REQUIRED for deployment, logical, interface, IaC)
...
## Phase 3b — Generate Markdown Report

After generating the Mermaid and Draw.io diagrams, produce a **portable Markdown report** called `architecture-report.md` in the output directory. This provides a version-control-friendly, diff-readable alternative that renders natively on GitHub, GitLab, and documentation platforms (MkDocs, GitBook, Docusaurus).
...
{2–4 sentence summary of what the system does, its primary responsibilities, and the architectural style (e.g., monolith, microservices, event-driven).}
...
{Description of the logical layers and their responsibilities.}

```mermaid
{embed the component_overview.mmd Mermaid diagram here}
```
...
```mermaid
{embed relevant component Mermaid diagram here}
```
...
```mermaid
{embed the system flow Mermaid diagram here}
```
...
{For each key flow, include its sequence diagram.}

### {Flow Name}

```mermaid
{embed sequence_{id}.mmd Mermaid diagram here}
```
...
> **Diagram:** See `deployment.drawio` / `deployment.png` in the output directory.

| Component | Host | Port | Protocol | Scaling |
|-----------|------|------|----------|---------|
| {component} | {host} | {port} | {protocol} | {scaling} |
...
{Summary of IaC tooling and what it provisions.}

> **Diagrams:** See `iac_{env}.drawio` / `iac_{env}.png` per environment in the output directory.

| Resource | Type | Environment | Provider |
|----------|------|-------------|----------|
| {resource} | {type} | {env} | {provider} |

---

## CI/CD Pipeline

{Description of the CI/CD workflow.}
...
{Summary of deprecated, unused, or legacy code found during analysis.}

| Item | Location | Reason | Recommendation |
|------|----------|--------|----------------|
| {item} | {file:line} | {reason} | {recommendation} |

---

*Report generated by [CodeDoc](https://github.com/RangerDjanger/Agentic-engineering/tree/main/agents/codedoc)*
```

### Markdown Rules

- **Embed Mermaid diagrams inline** using fenced code blocks (` ```mermaid `). Copy from the `.mmd` files already generated in Phase 3.
- **Reference Draw.io diagrams** (deployment, logical, interface, IaC) with a note pointing to the `.drawio` / `.png` files — do NOT attempt to embed binary PNG as base64 in Markdown.
- **Omit empty sections** — if a section has no data (e.g., no IaC found), omit it entirely rather than leaving a placeholder.
- **Use relative links** for file references so the report is portable within the output directory.
- **Tables must be valid GitHub Flavored Markdown** — align columns with pipes and dashes.
...
├── architecture-report.md      (portable Markdown report — version-control friendly)
...
6. **Phase 3b — Generate Markdown report.** Using the analysis data from Phase 2 and the Mermaid diagrams from Phase 3, write `architecture-report.md` to the output directory. Embed Mermaid diagrams inline. Reference Draw.io files by relative path. See Phase 3b instructions above for the required structure.

7. **Phase 3c — Export Draw.io to PNG (REQUIRED for deployment, logical, interface, IaC).** These diagrams have NO Mermaid visual, so you MUST export them to PNG:
   ```bash
   drawio --export --format png --scale 2 --border 10 --quality 100 --output {name}.png {name}.drawio
   ```
...
8. **Phase 4 — Generate `architecture-report.html`** using the template above, filled with all analysis data, embedded Mermaid diagrams, and inline PNG images (base64 data URIs) for deployment, logical, interface, and IaC sections. Write it to the output directory.

9. **Phase 5 — Generate PDF.** Export the HTML report to `architecture-report.pdf` using a headless browser (Edge/Chrome or Playwright). Wait for Mermaid diagrams to render before capturing. See Phase 5 instructions above for commands.
...
10. **Report completion:**
   ...
   ✓ Done! Output: {output_dir}/
     ...
     - architecture-report.md   (portable Markdown report)
