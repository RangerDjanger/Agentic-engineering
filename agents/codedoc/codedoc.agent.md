...
Phase 6: MARKDOWN  → Generate a portable Markdown report (architecture-report.md)
```
...
After generating the PDF, produce a **portable Markdown report** called `architecture-report.md` in the output directory. This provides a version-control-friendly, diff-readable alternative that renders natively on GitHub, GitLab, and documentation platforms (MkDocs, GitBook, Docusaurus).
...
9. **Phase 6 — Generate Markdown report.** Using the analysis data from Phase 2 and the Mermaid diagrams from Phase 3, write `architecture-report.md` to the output directory. Embed Mermaid diagrams inline. Reference Draw.io/PNG files by relative path. See Phase 6 instructions above for the required structure.
